...
void damppole(real r, real alphai, real alphak, real* dmpi, real* dmpk, real* dmpik)
{
    ...
    if constexpr (PenType == PenTyp::Gordon1) {
        ...
        dmpi[0] = 1 - (1 + (real)0.5*dampi)*expi;
        dmpi[1] = 1 - (1 + dampi + (real)0.5*dampi2)*expi;
        dmpi[2] = 1 - (1 + dampi + (real)0.5*dampi2 + dampi3/6)*expi;
        dmpi[3] = 1 - (1 + dampi + (real)0.5*dampi2 + dampi3/6 + dampi4/30)*expi;
        dmpi[4] = 1 - (1 + dampi + (real)0.5*dampi2 + dampi3/6 + 4*dampi4/105 + dampi5/210)*expi;
        if (diff < eps) {
            dmpk[0] = dmpi[0];
            dmpk[1] = dmpi[1];
            dmpk[2] = dmpi[2];
            ...
            dmpk[0] = 1 - (1 + (real)0.5*dampk)*expk;
            dmpk[1] = 1 - (1 + dampk + (real)0.5*dampk2)*expk;
            dmpk[2] = 1 - (1 + dampk + (real)0.5*dampk2 + dampk3/6)*expk;
            dmpk[3] = 1 - (1 + dampk + (real)0.5*dampk2 + dampk3/6 + dampk4/30)*expk;
            dmpk[4] = 1 - (1 + dampk + (real)0.5*dampk2 + dampk3/6 + 4*dampk4/105 + dampk5/210)*expk;
        }
        ...
        if (diff < eps) {
            ...
            dmpik[1] = 1 - (1 + dampi + (real)0.5*dampi2 + 7*dampi3/48 + dampi4/48)*expi;
            dmpik[2] = 1 - (1 + dampi + (real)0.5*dampi2 + dampi3/6 + dampi4/24 + dampi5/144)*expi;
            dmpik[3] = 1 - (1 + dampi + (real)0.5*dampi2 + dampi3/6 + dampi4/24 + dampi5/120 + dampi6/720)*expi;
            dmpik[4] = 1 - (1 + dampi + (real)0.5*dampi2 + dampi3/6 + dampi4/24 + dampi5/120 + dampi6/720 + dampi7/5040)*expi;
            if constexpr (rorder >= 11) {
                dampi8 = dampi4 * dampi4;
                dmpik[5] = 1 - (1 + dampi + (real)0.5*dampi2 + dampi3/6 + dampi4/24 + dampi5/120 + dampi6/720 + dampi7/5040 + dampi8/45360)*expi;
            }
        }
        else {
            ...
            termi2 = termi * termi;
            termk2 = termk * termk;
            dmpik[0] = 1 - termi2*(1 + 2*termk + (real)0.5*dampi)*expi
                         - termk2*(1 + 2*termi + (real)0.5*dampk)*expk;
            dmpik[1] = 1 - termi2*(1+dampi+(real)0.5*dampi2)*expi
                         - termk2*(1+dampk+(real)0.5*dampk2)*expk
                         - 2*termi2*termk*(1+dampi)*expi
                         - 2*termk2*termi*(1+dampk)*expk;
            dmpik[2] = 1 - termi2*(1 + dampi + (real)0.5*dampi2 + dampi3/6)*expi
                         - termk2*(1 + dampk + (real)0.5*dampk2 + dampk3/6)*expk
                         - 2*termi2*termk*(1 + dampi + dampi2/3)*expi
                         - 2*termk2*termi*(1 + dampk + dampk2/3)*expk;
            dmpik[3] = 1 - termi2*(1 + dampi + (real)0.5*dampi2 + dampi3/6 + dampi4/30)*expi
                         - termk2*(1 + dampk + (real)0.5*dampk2 + dampk3/6 + dampk4/30)*expk
                         - 2*termi2*termk*(1 + dampi + 2*dampi2/5 + dampi3/15)*expi
                         - 2*termk2*termi*(1 + dampk + 2*dampk2/5 + dampk3/15)*expk;
            dmpik[4] = 1 - termi2*(1 + dampi + (real)0.5*dampi2 + dampi3/6 + 4*dampi4/105 + dampi5/210)*expi
                         - termk2*(1 + dampk + (real)0.5*dampk2 + dampk3/6 + 4*dampk4/105 + dampk5/210)*expk
            ...
            if constexpr (rorder >= 11) {
                ...
                dmpik[5] = 1 - termi2*(1 + dampi + (real)0.5*dampi2 + dampi3/6 + 5*dampi4/126 + 2*dampi5/315 + dampi6/1890)*expi
                             - termk2*(1 + dampk + (real)0.5*dampk2 + dampk3/6 + 5*dampk4/126 + 2*dampk5/315 + dampk6/1890)*expk
    ...
    // core-valence charge penetration damping for Gordon f2
        ...
        dmpi[3] = 1 - (1 + dampi + real(0.4)*dampi2 + dampi3/15)*expi;
        if (diff < eps) {
            dmpk[0] = dmpi[0];
            dmpk[1] = dmpi[1];
            dmpk[2] = dmpi[2];
            ...
            dmpk[3] = 1 - (1 + dampk + real(0.4)*dampk2 + dampk3/15)*expk;
        }
        ...
        if (diff < eps) {
            dampi6 = dampi3 * dampi3;
            dmpik[0] = 1 - (1 + (real)0.5*dampi)*expi;
            dmpik[1] = 1 - (1 + dampi + (real)0.5*dampi2)*expi;
            dmpik[2] = 1 - (1 + dampi + (real)0.5*dampi2 + dampi3/6)*expi;
            dmpik[3] = 1 - (1 + dampi + (real)0.5*dampi2 + dampi3/6 + dampi4/30)*expi;
            dmpik[4] = 1 - (1 + dampi + (real)0.5*dampi2 + dampi3/6 + 4*dampi4/105 + dampi5/210)*expi;
            if constexpr (rorder >= 11) {
                dmpik[5] = 1 - (1 + dampi + (real)0.5*dampi2 + dampi3/6 + 5*dampi4/126 + 2*dampi5/315 + dampi6/1890)*expi;
            }
        }
        else {
            dampk4 = dampk2 * dampk2;
            ...
            dmpik[3] = 1 - termi*(1 + dampi + real(0.4)*dampi2 + dampi3/15)*expi 
                         - termk*(1 + dampk + real(0.4)*dampk2 + dampk3/15)*expk;
