...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i <= stringlen; i++) {
        letter = string[i];
        if (letter == 'G') dosystem = true;
        if (letter == 'P') doparam = true;
        if (letter == 'E') doenergy = true;
        if (letter == 'A') doatom = true;
        if (letter == 'L') dolarge = true;
        if (letter == 'D') dodetail = true;
        if (letter == 'M') domoment = true;
        if (letter == 'V') dovirial = true;
        if (letter == 'C') doconect = true;
    }

    // set option control flag to save forces or induced dipoles
    dosave = false;
    optinit();
...
//       if (doconect)  call connyze (active)
    ...
    while (!informAbort) {
        ...
        printf("frame %d\n", frame);
        for (int i = 0; i < n; i++) {
            printf("  %22.16f%22.16f%22.16f   %5d\n", x[i], y[i], z[i], type[i]);
        }
        printf("\n\n\n");
...
//       if (digits >= 6)  fstr(31:38) = '7x,f18.6'
//       if (digits >= 8)  fstr(31:38) = '5x,f20.8'
//       if (abs(einter) >= 1.0d10)  fstr(34:34) = 'd'
//       if (nmol>1 and nmol<n and !use_ewald) then
...
//       if (digits >= 6)  fstr(32:39) = '6x,f18.6'
//       if (digits >= 8)  fstr(32:39) = '4x,f20.8'
//       if (abs(energy) >= 1.0d10)  fstr(35:35) = 'd'
...
//       end if
//       if (use_repel and (ner!=0 or er!=0.0d0)) then
//          if (abs(er) < 1.0d10) then
...
//       end if
//       if (use_disp and (nedsp!=0 or edsp!=0.0d0)) then
//          fstr = '('' Dispersion'',17x,'//form1//')'
//          write (iout,fstr)  edsp,nedsp
//       end if
//       if (use_charge and (nec!=0 or ec!=0.0d0)) then
//          if (abs(ec) < 1.0d10) then
...
//       end if
//       if (use_chgdpl and (necd!=0 or ecd!=0.0d0)) then
//          if (abs(ecd) < 1.0d10) then
...
//       end if
//       if (use_dipole and (ned!=0 or ed!=0.0d0)) then
//          if (abs(ed) < 1.0d10) then
...
//       end if
//       if (use_mpole and (nem!=0 or em!=0.0d0)) then
//          if (abs(em) < 1.0d10) then
...
//       end if
//       if (use_polar and (nep!=0 or ep!=0.0d0)) then
//          if (abs(ep) < 1.0d10) then
...
//       end if
//       if (use_chgtrn and (nect!=0 or ect!=0.0d0)) then
//          if (abs(ect) < 1.0d10) then
...
//                end if
//                j = 0
//                if (itors1(1,i) != 0.0d0) then
//                   j = j + 1
//                   fold(j) = 1
//                   ampli(j) = itors1(1,i)
//                   phase(j) = itors1(2,i)
//                end if
//                if (itors2(1,i) != 0.0d0) then
//                   j = j + 1
//                   fold(j) = 2
//                   ampli(j) = itors2(1,i)
//                   phase(j) = itors2(2,i)
//                end if
//                if (itors3(1,i) != 0.0d0) then
//                   j = j + 1
//                   fold(j) = 3
//                   ampli(j) = itors3(1,i)
//                   phase(j) = itors3(2,i)
...
//                end if
//                j = 0
//                if (tors1(1,i) != 0.0d0) then
//                   j = j + 1
//                   fold(j) = 1
//                   ampli(j) = tors1(1,i)
//                   phase(j) = tors1(2,i)
//                end if
//                if (tors2(1,i) != 0.0d0) then
//                   j = j + 1
//                   fold(j) = 2
//                   ampli(j) = tors2(1,i)
//                   phase(j) = tors2(2,i)
//                end if
//                if (tors3(1,i) != 0.0d0) then
//                   j = j + 1
//                   fold(j) = 3
//                   ampli(j) = tors3(1,i)
//                   phase(j) = tors3(2,i)
//                end if
//                if (tors4(1,i) != 0.0d0) then
//                   j = j + 1
//                   fold(j) = 4
//                   ampli(j) = tors4(1,i)
//                   phase(j) = tors4(2,i)
//                end if
//                if (tors5(1,i) != 0.0d0) then
//                   j = j + 1
//                   fold(j) = 5
//                   ampli(j) = tors5(1,i)
//                   phase(j) = tors5(2,i)
//                end if
//                if (tors6(1,i) != 0.0d0) then
//                   j = j + 1
//                   fold(j) = 6
//                   ampli(j) = tors6(1,i)
//                   phase(j) = tors6(2,i)
...
//                   radj = rad(j)
//                   rad4j = rad4(j)
//                   if (radsiz == 'DIAMETER') then
//                      radj = 2.0d0 * radj
//                      rad4j = 2.0d0 * rad4j
//                   end if
//                   if (radtyp == 'SIGMA') then
//                      radj = radj / twosix
//                      rad4j = rad4j / twosix
//                   end if
//                   if (reduct(j) == 0.0d0) then
...
//          header = true
//          do i = 1, nangle
//             if (aflx(1,i)!=0.0d0 or aflx(2,i)!=0.0d0 or
//      &          abflx(1,i)!=0.0d0 or abflx(2,i)!=0.0d0) then
//                ia = iang(1,i)
//                ib = iang(2,i)
//                ic = iang(3,i)
//                if (active(ia) or active(ib) or active(ic)) then
...
//                end if
//                j = 0
//                if (kst(4,i) != 0.0d0) then
//                   j = j + 1
//                   fold(j) = 1
//                   ampli(j) = kst(4,i)
//                   phase(j) = tors1(2,k)
//                end if
//                if (kst(5,i) != 0.0d0) then
//                   j = j + 1
//                   fold(j) = 2
//                   ampli(j) = kst(5,i)
//                   phase(j) = tors2(2,k)
//                end if
//                if (kst(6,i) != 0.0d0) then
//                   j = j + 1
//                   fold(j) = 3
//                   ampli(j) = kst(6,i)
//                   phase(j) = tors3(2,k)
